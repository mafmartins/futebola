...
class Epoca(models.Model):
    ...
    def lista_jogs(self, order_by):
        ...
        cursor.execute('SELECT *, vitorias*'+str(self.valor_vitoria)+' + empates*'+str(self.valor_empate)+' + jogos*'+str(self.valor_participacao)+' + golos*'+str(self.valor_golos)+' + assistencias*'+str(self.valor_assistencias)+' - COALESCE(penalizacao,0) AS pontuacao, golos+assistencias AS combinado FROM ( SELECT `futebola_jogador`.`nome`, `futebola_jogador`.`jogador_id`, `penalizacoes`.`penalizacao`, SUM(( CASE WHEN equipa = "equipa_a" AND resultado_a > resultado_b THEN 1 WHEN equipa = "equipa_b" AND resultado_b > resultado_a THEN 1 ELSE 0 END )) AS vitorias, SUM(( CASE WHEN equipa = "equipa_a" AND resultado_a < resultado_b THEN 1 WHEN equipa = "equipa_b" AND resultado_b < resultado_a THEN 1 ELSE 0 END )) AS derrotas, SUM(( CASE WHEN resultado_a = resultado_b THEN 1 ELSE 0 END )) AS empates, SUM(golos) AS golos, SUM(assistencias) AS assistencias, COUNT(ficha_id) AS jogos FROM `futebola_ficha_de_jogo` INNER JOIN `futebola_jogo` ON (`futebola_ficha_de_jogo`.`jogo_id` = `futebola_jogo`.`jogo_id`) INNER JOIN `futebola_epoca` ON (`futebola_jogo`.`epoca_id` = `futebola_epoca`.`epoca_id`) INNER JOIN `futebola_jogador` ON (`futebola_ficha_de_jogo`.`jogador_id` = `futebola_jogador`.`jogador_id`) LEFT JOIN (SELECT `futebola_penalizacao`.`jogador_id`, SUM(`futebola_penalizacao`.`valor`) AS penalizacao FROM `futebola_penalizacao` WHERE `futebola_penalizacao`.`epoca_id` = '+str(self.epoca_id)+' GROUP BY `futebola_penalizacao`.`jogador_id`) AS penalizacoes ON (`futebola_ficha_de_jogo`.`jogador_id` = `penalizacoes`.`jogador_id`) WHERE ( `futebola_epoca`.`numeracao_epoca` = '+str(self.numeracao_epoca)+' AND now() > date_add(data, INTERVAL 22 HOUR) ) GROUP BY `futebola_jogador`.`nome`,`futebola_jogador`.`jogador_id`, `penalizacoes`.`penalizacao` ) AS jogadores ORDER BY '+order_by)
